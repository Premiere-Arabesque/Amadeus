...
Amadeus是一个roleplay Agent引擎。你给它一份角色设定，它会让这个角色像真人一样过日子——每天制定计划，按计划做事，跟真实的互联网交互，积累记忆和经历，并在合适的时刻主动联系你。

这个引擎可以驱动任何类型的角色：一个理想的AI伴侣、一个真人的数字分身、甚至一只电子宠物。区别只在于你输入的设定。

---


## 效果展示
(此demo的人设及相关测试数据来自于claude生成)
...
最后角色在刷小红书的时候触景生情,想要给对方发消息,纠结了好一会儿，最后鼓起勇气打开微信——

删删改改，发出了一条消息。



这不是定时推送，不是随机触发。是她在刷手机的时候，因为看到的内容联想到了对方，所以想找对方聊天。
...
- 更多消息渠道接入
- 前端界面
- Roleplay agent的幻觉问题,可以看到在demo里面Roleplay会编造出一些不存在的内容,但是这个是LLM本身的内在缺陷,未来会继续打磨尽量让幻觉降低到最小
